    ...
    displayMatchPairs(question) {
        ...
        const updateConnectionLine = () => {
            if (!selectedItemButton || !selectedValueButton) {
                // If we don't have both items selected, hide any existing line
                if (connectionLine) {
            ...
            // Make sure we have valid coordinates before drawing the line
            ...
            // Only show the line if we have a reasonable length
            if (length < 10) {
                connectionLine.style.opacity = '0';
                return;
            }
