class HangmanGame {
    ...
    displayCategorize(question) {
        ...
        let attemptedCategorizationsCount = 0;
        ...
        question.categories.forEach(category => {
            ...
            categoryDiv.addEventListener('drop', (e) => {
                ...
                const droppedItemElement = document.getElementById(`item-${itemId}`);
                ...
                if (droppedItemElement && !droppedItemElement.classList.contains('categorized')) {
                    // Move the item to the category container
                    categoryDiv.appendChild(droppedItemElement);
                    droppedItemElement.classList.add('categorized');
                    
                    const isCorrect = targetCategory === correctCategory;
                    attemptedCategorizationsCount++;

                    if (isCorrect) {
                        droppedItemElement.classList.add('correct');
                        ...
                        droppedItemElement.classList.add('incorrect');
                        // Show the correct category as feedback
                        const feedback = document.createElement('div');
                        feedback.classList.add('categorize-feedback');
                        feedback.textContent = `This belongs in: ${correctCategory}`;
                        droppedItemElement.appendChild(feedback);
                        
                        // Trigger wrong answer
                        this.handleAnswer(false);
                    }

                    // Make the item draggable again after a short delay
                    setTimeout(() => {
                        droppedItemElement.draggable = true;
                    }, 500);
                }
            });
        ...
        this.shuffleArray(items).forEach(item => {
            ...
            itemDiv.addEventListener('dragstart', (e) => {
                ...
                // Remove any previous feedback when starting a new drag
                const feedback = itemDiv.querySelector('.categorize-feedback');
                if (feedback) {
                    feedback.remove();
                }
                itemDiv.classList.remove('correct', 'incorrect', 'categorized');
            });
