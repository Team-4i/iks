class HangmanGame {
    ...
    displayMatchPairs(question) {
        ...
        let connectionLine = null; // Reference to connection line element
            ...
            if (selectedItemButton && selectedValueButton) {
                ...
                setTimeout(() => {
                    ...
                    if (isCorrect) {
                        ...
                        // Keep connection line for correct matches
                        if (connectionLine) {
                            connectionLine.style.backgroundColor = '#2ecc71'; // Green for correct
                            connectionLine.style.height = '3px'; // Make it slightly thicker
                            connectionLine.style.opacity = '1'; // Ensure it's visible
                            // Don't reset the connection line to null so it stays visible
                            // Don't call resetSelection here
                            
                            // Instead of resetting, just update the classes
                            if (selectedItemButton) selectedItemButton.classList.remove('selected');
                            if (selectedValueButton) selectedValueButton.classList.remove('selected');
                            
                            // Create a new connection line for the next pair
                            connectionLine = null;
                        ...
                        }, 800); // Longer delay to see the feedback
                    }
                }, 300); // Short delay to see the connection
            }
        };
