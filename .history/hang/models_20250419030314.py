...
class HangmanGame(models.Model):
    ...
    points = models.IntegerField(default=0)          # Track points earned in the game
...
class PlayerStats(models.Model):
    ...
    total_points = models.IntegerField(default=0)         # Total points earned across all games
    best_points = models.IntegerField(default=0)          # Highest points in a single game
...
class BaseQuestion(models.Model):
    ...
    # Adding source tracking fields
    main_topic = models.ForeignKey(MainTopic, on_delete=models.SET_NULL, null=True, blank=True, related_name="%(class)s_questions")
    sub_topic = models.ForeignKey(SubTopic, on_delete=models.SET_NULL, null=True, blank=True, related_name="%(class)s_questions")
    topic = models.ForeignKey(Topic, on_delete=models.SET_NULL, null=True, blank=True, related_name="%(class)s_questions")
    chapter = models.ForeignKey(Chapter, on_delete=models.SET_NULL, null=True, blank=True, related_name="%(class)s_questions")
